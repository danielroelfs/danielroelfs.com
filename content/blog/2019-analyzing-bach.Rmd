...
tags:
  ...
  - R
  - Bach
description: '(DRAFT) Analyzing Bach'
...
A little while ago I was watching a documentary series on Dutch television about one of the most beloved composers in the Netherlands: Johann Sebastian Bach. I had  
...
```{r echo=FALSE}
load("../../static/blog/2019-analyzing-bach_data/scraped_data.Rdata")
```
```{r}
str(scraped_data)
```
...
The first descritive I wanted to see was what kind of work Bach wrote and in what numbers. 
...
summ_key_cat1 <- merged_data %>%
  group_by(category1, Key, .drop = FALSE) %>%
  summarise(n = n()) %>%
  filter(nchar(Key) > 1) %>%
...
summ_key_cat2 <- merged_data %>%
  group_by(category2, Key, .drop = FALSE) %>%
  summarise(n = n()) %>%
  filter(nchar(Key) > 1) %>%
  mutate(Key = sub("\u266D", "b", Key),
         Key = sub("\U266F", "#", Key),
...
plotdat <- rbind(summ_key_cat1 %>% rename(category2 = category1),summ_key_cat2) %>%
  cbind(gr = c(rep(1,nrow(summ_key_cat1)),rep(2,nrow(summ_key_cat2))))

ggplot(plotdat, aes(x = Key, y = reorder(category2,n), fill = n)) +
  #facet_wrap(~ gr, dir = "v", scales='free_y') +
  geom_tile() +
  geom_text(aes(label = n), family = "Alegreya", color = "white", size = 3) +
  #geom_hline(yintercept = 0.5:8.5, color = "grey30") +
  geom_hline(yintercept = 8.5, color = "#FFFFF0", size = 1) + 
  #geom_vline(xintercept = 0.5:25.5, color = "grey30") +
  labs(x = NULL,
       y = NULL,
       fill = NULL) +
...
save(places, file = '../../static/blog/2019-analyzing-bach_data/places.Rdata')
```

```{r echo=FALSE, fig.width=12}
load('../../static/blog/2019-analyzing-bach_data/places.Rdata')
...
ggplot(places_plot) +
  geom_rect(aes(xmin = year_from + 0.5, xmax = year_to + 0.5, ymin = 0, ymax = 60, fill = reorder(city,year_from)),
            alpha = 0.75) +
  #geom_bar(BWVperyear, mapping = aes(x = year, y = n), fill = "black", stat = "identity") +
  stat_smooth(data = BWVperyear, mapping = aes(x = year + 0.5, y = n, group = 1), fill = "black", geom = 'area', method = 'loess', span = 0.225, alpha = 0.75) +
...
#bw_map <- get_googlemap(center = c(11, 52), zoom = 7,
#                        color = "bw",
