...
```{r eval=FALSE}
data <- read.table("AllChannels_ERP.txt", sep = "\t", header = FALSE)
```
...
The next piece of code is probably a bit messy, but again, it works. Here I’m calculating the confidence interval and then transforming it from the interval relative to the mean to the ab- solute values representing the upper and lower boundaries of the confidence interval. Here a confidence interval of 95% is used.
...
for (i in 1:length(Conditions)){
  ...
  for (j in 1:ncol(times)) {
    CI <- Rmisc::CI(as.numeric(unlist(MeanERPs %>% 
                                        filter(Condition == Conditions[i]) %>%
                                        select(as.character(times[1,j])))), 
                    ci = 0.95)
...
The last step before plotting is to transform the data into something that `ggplot` can understand. The `melt()` function works, but I prefer to use the `gather()` function from the `{tidyr}` package. In the future I might rewrite this to the new `pivot_longer()` function.
...
I take the colors from our own  `{normentR}` package. After running this chunk, we have a beautiful ERP with a nice confidence interval. I hope this was helpful and informative. Good luck!

```{r fig.width=12, fig.height=6}
colors <- norment_pal(palette = "logo")(2)

ERP <- ggplot(ERPplot, aes(x = Time, y = Amplitude, 
