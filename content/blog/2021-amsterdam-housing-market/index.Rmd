...
```{r plot-overpay, warning=FALSE, fig.width=8, fig.height=6}
...
```{r plot-tightness, warning=FALSE, fig.width=8, fig.height=6}
data %>% 
  filter(type != "Total") %>% 
  group_by(type) %>% 
  mutate(n_total = sum(n_sold),
...
```{r plot-n-sales, warning=FALSE, fig.width=10, fig.height=7}
data %>% 
  filter(type != "Total") %>% 
...
```{r plot-n-homes, message=FALSE, warning=FALSE, fig.width=10, fig.height=8}
decades <- tibble(
  x1 = seq(1920,2020,10),
  x2 = x1 + 10
) %>% 
...
```{r plot-housing-n_mutations, warning=FALSE, fig.width=10, fig.height=9}
...
```{r net-shift-to-renters, message=FALSE, fig.width=10, fig.height=6}
...
```{r mutations-table, out.width="60%"}
house_mutations %>% 
...
```{r plot-asking-index-increase, message=FALSE, warning=FALSE, fig.width=10, fig.height=6}
asking_start <- data %>% 
  filter(type != "Total",
         date == min(date)) %>% 
