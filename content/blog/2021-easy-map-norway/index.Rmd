...
```{r get-cities-locations, eval=FALSE}
...
```{r save-cities-geo, echo=FALSE, eval=FALSE}
write_rds(hospitals_df, "norway_hospitals.rds")
```

```{r load-cities-geo, echo=FALSE}
...
```{r show-leaflet-widget, echo=FALSE, eval=FALSE}
map <- map_sf %>% 
  leaflet() %>% 
  addProviderTiles(providers$Esri.WorldStreetMap) %>%
  addPolygons(
